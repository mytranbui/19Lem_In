...
/*   Updated: 2021/06/13 17:14:49 by mbui             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	t_link* tmp = l->hm[i]->links;

	l->hm[i]->visited = 1;
	//   ft_printf("%s Visited %d \n", l->hm[i]->key , l->hm[i]->visited);
	while (tmp != NULL) {
		int nextnode = tmp->node->index;
		if (i == l->node_end->index)
		{
			l->hm[l->i_end]->dist=0;
			l->hm[l->i_end]->visited=0;
			DFS(l, l->node_start->index);
		}
		if (l->hm[nextnode]->visited == 0)
		{
			// p = add_path(p, l->hm[nextnode]);
			get_info(l, nextnode, i);
			i = nextnode;
			DFS(l, nextnode);
		}
		tmp = tmp->next;
